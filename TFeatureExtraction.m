function [TFeature]=TFeatureExtraction(dataLead,QFeature,sampFreq,timeAxis)
...
for var2=1:1:length(RInd)
    if (heartRate > 60)
        ...
        STPoint(var2) = dataLead(STPointInd(var2));
    else
...
STSlope = zeros(1,length(RInd));
for var2=1:1:length(RInd)
    STSlope(var2) = (STPoint(var2) - S(var2))/((STPointInd(var2) - QRSOff(var2))/sampFreq);
end

STShape = zeros(1,length(RInd));

P1 = zeros(1,length(RInd));
P2 = zeros(1,length(RInd));
P3 = zeros(1,length(RInd));

for var2=1:1:length(RInd)
    P1(var2) = dataLead(ceil(QRSOff(var2) - 60));
    P2(var2) = dataLead(ceil(QRSOff(var2)));
    P3(var2) = dataLead(ceil(QRSOff(var2) + 60));
