function [QFeature]=QFeatureExtractionNew(dataLead,sampFreq,timeAxis)
...
thresh = nonLinTrfm > (0.3*max(nonLinTrfm(round(length(nonLinTrfm)/4):round(3*length(nonLinTrfm)/4))));
...
for noBeat=2:1:length(threshIndLeft)
    if (threshIndLeft(noBeat) - threshIndRight(noBeat - 1) > tallTWave)
...
% qrsOn(noBeat + 1) = 0;
% qrsOff(noBeat + 1) = 0;
% qrsOnOff = zeros(1,length(nonLinTrfm));
% 
% % qrsOnOff = zeros(1,length(nonLinTrfm));
% for var2=1:1:length(nonLinTrfm)
%     if ((qrsOn(b1) == var2) && (b1 < length(qrsOn)))
%         qrsOnOff(var2) = -2*max(dataLead40);
%         b1 = b1 + 1;
%     elseif ((qrsOff(b2) == var2) && (b2 < length(qrsOff)))
%         qrsOnOff(var2) = -2*max(dataLead40);
%         b2 = b2 + 1;
%     elseif ((qrsOn(b1) == 0) && (b1 < length(qrsOn)))
%         qrsOnOff(var2) = 0;
%         b1 = b1 + 1;
%     elseif ((qrsOff(b2) == 0) && (b2 < length(qrsOff)))
%         qrsOnOff(var2) = 0;
%         b2 = b2 + 1;
%     else 
%         qrsOnOff(var2) = 0;
...
for var1=1:1:length(qrsOn)
    ...
    [RPeak(var1) RInd(var1)] = max(dataLeadOrg(qrsOn(var1):qrsOff(var1)));
    RInd(var1) = qrsOn(var1) + RInd(var1) - 1;
    RPeakFilt(var1) = dataLeadOrg(RInd(var1));

    [SPeak(var1) SInd(var1)] = min(dataLeadOrg(RInd(var1):qrsOff(var1)));
    SInd(var1) = SInd(var1) + RInd(var1) - 1;
    SPeakFilt(var1) = dataLeadOrg(SInd(var1));

    [QPeak(var1) QInd(var1)] = min(dataLeadOrg(qrsOn(var1):RInd(var1)));
    QInd(var1) = QInd(var1) + qrsOn(var1) - 1;
    QPeakFilt(var1) = dataLeadOrg(QInd(var1));
end
%             for var1=1:1:length(qrsOn)
%                 flagR = 0;
%                 flagR1 = 0;
...
% 
%                 if ((flagR==0) && (flagR1==1))
%                     [RPeak(var1) RInd(var1)] = max(dataLeadOrg(RLeft:RRight));
